...
public class Hooks {
    ...
    private static byte[] getTrampolineArray(long art_method, long entry_point) {
        ...
        return switch (CURRENT_INSTRUCTION_SET) {
            ...
            //TODO: riscv64
            case RISCV64 -> new byte[]{
                    // auipc t0, 0
                    // ld a0, 16(t0)   ; a0 = art_method
                    // ld t6, 24(t0)   ; t6 = entry_point
                    // jalr zero, 0(t6)
                    (byte) 0x97, 0x02, 0x00, 0x00,
                    0x03, (byte) 0xB5, 0x02, 0x01,
                    (byte) 0x83, (byte) 0xBF, (byte) 0x82, 0x01,
                    0x67, (byte) 0x80, 0x0F, 0x00,
                    m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],   //  art_method
                    e[0], e[1], e[2], e[3], e[4], e[5], e[6], e[7]    //  entry_point
            };
